	...
	private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getPathInfo().equals("/Form")) {
			...
			try {
				...
				Gson gson = new Gson();
				EmployeeInputBean emp = gson.fromJson(jb.toString(), EmployeeInputBean.class);
				
				HttpSession session = request.getSession();
				emp.setCreateBy((String) session.getAttribute("username"));
				
				Context context = new InitialContext();
				EmployeeRemote remote = (EmployeeRemote) context.lookup("BMS.EmployeeEJB");

				out.println(remote.addEmployee(emp));
